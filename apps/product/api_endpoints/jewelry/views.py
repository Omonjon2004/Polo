...
from apps.product.api_endpoints.jewelry.serializers \
    import JewelrySerializer
...
class JewelryListCreateAPIView(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if weight:
            queryset = (queryset.annotate(
                sim=TrigramSimilarity('weight', weight)).
                        filter(sim__gte=0.2).order_by('sim')
                        )
        if price:
            queryset = (queryset.annotate(
    ...
    @swagger_auto_schema(
        manual_parameters=[
            ...
            openapi.Parameter("weight", openapi.IN_QUERY,
...
class JewelryRetrieveUpdateDestroyAPIView\
...
__all__ = [
    'JewelryListCreateAPIView',
    'JewelryRetrieveUpdateDestroyAPIView'
]
